---
name: deploy
description: 用 wink-sftp 把本地目录通过 SFTP 部署到远程服务器。当用户要「部署 / 发布 / 上传到服务器」、「把 dist 传到远程」、配置 sftp.json、或排查 wink-sftp 部署结果时使用。封装「先预演、人工确认、再真跑、按结构化结果判定成败」的安全流程。
---

# wink-sftp 部署

`wink-sftp` 是一个把本地目录经 SFTP 全量传输到远程服务器的 CLI，支持传输前后执行远程命令。本 Skill 规约其**安全调用流程**，供人机协作与 agent 调用。
...
写一个 `sftp.json`（也支持 `.yaml/.yml`，按扩展名识别），用 `-c` 指定：
...
```jsonc
{
    ...
    "sftpOptions": {
        ...
        "ignore": ["*.log", "tmp/"], // gitignore 风格内联忽略，与 .winksftpignore 合并
...
```bash
wink-sftp -c sftp.json --dry-run --json          # 1) 预演，给用户确认
wink-sftp -c sftp.json --json                    # 2) 确认后真跑
wink-sftp -c sftp.json --env prod --dry-run --json  # 选中 prod 环境（覆盖叠加在基础配置之上）
```

> **`.winksftpignore`**：在 `local` 根放一个 gitignore 风格的忽略文件，自动生效（含目录整体剪枝），与 `sftpOptions.ignore` 合并。
>
...
```bash
wink-sftp -l dist -r /var/www/app \
...
实跑默认向 `~/.wink-sftp/audit.log` 追加一条记录（时间 / 主机 / 用户 / 结果 / 计数）。可用 `--audit-log <path>` 改路径、`--no-audit` 关闭。排查「谁在何时改了哪台机器」可查此文件。
