...
- `docs/ROADMAP.md` — 定位与愿景、现状能力（v1.0）、前瞻路线（规模化与可视化）。
...
Long-term vision: an agentless "SSH 快捷操作入口" for solo devs and AI agents, covering a standard-stack server's full lifecycle — provision (install/configure nginx/redis/mysql/docker/node/jdk/python via curated recipes) → deploy → status/logs → edit/service. 单机全生命周期已打通（v1.0），下一步是多机舰队与可视化。

## Overview

`@xwink/ops` (`winkops`) is an agentless "SSH 快捷操作入口" CLI: deploy (SFTP), ops primitives (exec/status/logs/ps/service/edit), and provision (declarative stack convergence). Published to npm as a single `winkops` binary (ncc bundle).
...
- `session.ts` — `SshSession`（`open`/`exec`/`stream` 流式/`sftp` 缓存/`close`/`raw`）+ `withSession`：通用 SSH 会话抽象，部署/下载/浏览/guard 的共同底座，也是编程式 API。`stream(command, {onStdout,onStderr})` 回调式实时吐数据块、返回 `StreamHandle`（`done`/`close`），支撑 `tail -f` 等长流。
...
- `ops.ts` — 运维原语（经 `SshSession`，需 connect 不需 local/remote）：`runExec`（远程执行，退出码非零作结构化结果不抛）、`status`（agentless 快照：一次 exec 采集 + 纯函数解析器 `parseLoadavg`/`parseMeminfo`/`parseDf` 归一化，best-effort）、`tailLogs`（`tail -n` + grep）、`followLogs`（`tail -f` 流式 + 行缓冲按行回调，`logs --follow`）、`streamExec`（流式远程执行，`exec --stream`）、`ps`（`ps -A` 采集 + 纯函数 `parsePs` 解析 + 客户端 grep 过滤）、`service`（服务管理，纯函数 `buildServiceCommand` 构造 systemd/pm2/docker 命令；`status` 只读放行、写动作经 `isWriteAction` 判定须 `yes` 确认 + 记审计，退出码非零不抛）。
- `edit.ts` — `edit`：用本地 `--file` 内容原子替换远程文件，复用 `guard`（备份→写入→`--validate`→`--reload`→失败回滚）。
- `provision.ts` — 环境初始化（单机）：按声明式 `stack`（配置文件字段）把服务器收敛到目标栈。recipe 契约用**纯函数**描述（`detect(options)` 检测命令 + `parse` 解析 + `converge(desired,state,options)` 收敛规划，便于 fixture 单测），编排器执行检测→算收敛步骤→预演（`--dry-run`）或执行（`--yes`，写护栏二选一）；幂等由 `converge`「已满足则空步骤」保证。recipe：`nodejs`(nvm)/`jdk`(sdkman)/`python`(pyenv)/`docker`(官方脚本) + `nginx`/`redis`/`mysql`（install + verify，redis/mysql 支持 `mode: docker|native`）；纯函数 `versionSatisfies`/`normalizeDesired`/`parseConfigs` + `RECIPES` 注册表。**含 secret（如 mysql rootPassword）的命令/输出由编排层按 secret 明文值统一脱敏（`collectSecrets`/`scrubSecrets`，命令+stdout+stderr+失败路径全覆盖），默认安全、绝不泄漏明文**。守护式 `configure`：任一组件 stack 对象可声明 `configure`（本地文件 → 远程，附可选 `validate`/`reload`），安装/已满足后逐条经 `guard`（备份→写→校验→reload→失败回滚）落地，与 `edit` 同一流水线；本地源经 SFTP 传输、明文不进命令。写动作记审计。
...
- `scanner.ts` — pure local FS scan + `.winkignore`（`loadIgnorePatterns`，gitignore 风格 glob）。
...
- `audit.ts` — 写操作审计：`recordAudit`（从 `ResolvedConfig` 取主机/用户/开关写一条记录，审计关闭跳过、写失败仅 warn 不中断，deploy/service 等写操作共用）+ 底层 `appendAudit` / `formatAuditLine` / `defaultAuditPath`（`~/.winkops/audit.log`）。
...
```bash
pnpm dev            # tsx src/index.ts -c wink.json（无需构建）
pnpm test           # vitest
...
**发布流程（先审查、分两步推）**：① **先用 `/code-review` 审查待发版内容**（自上个稳定 tag 以来的 diff），问题全部修完；② `pnpm run release` 本地改版本/CHANGELOG/提交/打 tag 并**只推 main**；③ 等 `ci.yml`（lint/format/typecheck/test/build/e2e/commitlint，Node 22/24 + Node 18 冒烟）**全部绿**后，再 `pnpm run release:tag` 推送 tag 触发 `release.yml` 发布 npm + GitHub Release。**绝不在 main 门禁通过前推 tag**——否则可能发布出门禁未过的版本；**也绝不跳过发布前 code-review**——`@xwink/ops` 发到 npm 不可逆，机器门禁拦不住逻辑/设计问题。审查放在推 main 前，是因为它的修复多需重新过门禁，前置可随门禁一次跑完，免得发版途中折返。门禁没过的处理：真实逻辑问题就 fix-forward（修复较大时补一次 review）、把 tag 移到修复后的绿提交（`git tag -f` 后显式 `git push origin v<x>`）再发；flaky 则 `gh run rerun <id> --failed` 重跑、tag 不动直接发。
...
- **`ignoreHidden`** 只对相对 `local` 的路径段做 `startsWith('.')` 判定；`.winkignore` / `sftpOptions.ignore` 走 `ignore` 包的 gitignore 匹配。
