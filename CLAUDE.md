...
`dev/e2e/`（`pnpm run e2e`，CI 一步）是**真**端到端：`server.ts` 进程内起一个 ssh2 SSH/SFTP 服务端（透传真实 fs + `/bin/sh` exec，仅测试用、不鉴权），`run.ts` spawn 真实 CLI 跑 deploy/pull/ls/密钥/密码/增量/忽略/多环境/失败退出码并断言。它覆盖 `vi.mock` 桩**绕不到的 commander 子命令参数解析**——正因如此 mock 测试漏掉过「子命令与根命令同名选项被 commander 归到父命令」的 bug。
