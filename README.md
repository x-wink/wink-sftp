...
![名称](https://img.shields.io/github/package-json/name/x-wink/winkops?style=for-the-badge)
![版本](https://img.shields.io/github/package-json/v/x-wink/winkops?style=for-the-badge&filename=package.json)
![关键字](https://img.shields.io/github/package-json/keywords/x-wink/winkops?style=for-the-badge)
![许可](https://img.shields.io/github/package-json/license/x-wink/winkops?style=for-the-badge)

![下载量](https://img.shields.io/npm/dt/%40xwink/ops?style=for-the-badge&logo=npm)
![大小](https://img.shields.io/bundlephobia/minzip/%40xwink/ops?style=for-the-badge&logo=npm)

![收藏](https://img.shields.io/github/stars/x-wink/winkops?style=flat-square&logo=github)
![问题](https://img.shields.io/github/issues/x-wink/winkops?style=flat-square&logo=github)
![请求](https://img.shields.io/github/issues-pr/x-wink/winkops?style=flat-square&logo=github)

---

## 这是什么

`winkops`（npm 包名 `@xwink/ops`）是一个零运行时配置的部署 CLI：把本地某个目录（通常是构建产物 `dist/`）通过 SFTP 全量上传到远程服务器，并可在传输前后执行远程命令（停服、解压、重启等）。
...
- 🧩 稳定的编程式 API：`import { SshSession, run, runMany, guard } from '@xwink/ops'` 供 Node 脚本集成
- 🙈 自动跳过隐藏文件，支持按路径排除与 `.winkignore`（gitignore 风格 glob）
...
```bash
# 作为项目开发依赖
pnpm add -D @xwink/ops
npm install -D @xwink/ops
...
```bash
npx winkops -l ./dist -r /apps/myapp -h 192.168.1.10 -p 22 -u root --connect-password 123456
```
...
```bash
npx winkops -l ./dist -r /apps/myapp -h 192.168.1.10 -p 22 -u root \
...
将参数写入 `wink.json` 并纳入版本管理，部署时只需指定路径：

```bash
npx winkops -c ./wink.json
```
...
>
> - 配置文件作为**基底**，命令行/编程式**显式设置**的字段（`--connect-host`、`--remote`、`--sftp-*` 等）会**深合并**覆盖其上；未显式设置的字段保留文件值。例如 `-c wink.json --remote /other` 只改 `remote`，连接信息仍取自文件。
...
| `sftpOptions.ignore`           | string[] | `[]`    | gitignore 风格忽略规则（与 `.winkignore` 合并）                      |
...
| `--audit-log <path>`            | —                              | 审计日志路径（默认 `~/.winkops/audit.log`）    |
| `--no-audit`                    | —                              | 禁用本地审计日志                               |
| `--help`                        | —                              | 显示帮助                                       |
| `-V, --version`                 | —                              | 显示版本号                                     |
...
```bash
# 列出远程目录（只读）
npx winkops ls /apps/myapp -h 192.168.1.10 -p 22 -u root --connect-password 123456

# 下载远程文件/目录到本地（目录递归镜像）
npx winkops pull -r /apps/myapp -l ./backup -h 192.168.1.10 -p 22 -u root --connect-password 123456

# 也可复用配置文件的连接信息（-c）
npx winkops ls -c ./wink.json
```
...
npx winkops exec 'systemctl is-active nginx' -h 1.2.3.4 -u root --connect-password '***' --json
# exec --stream：实时流式输出（适合长流/大输出，--json 无效）
npx winkops exec 'journalctl -u nginx -f' --stream -c ./wink.json

# status：agentless 资源/健康快照（CPU 核数/负载/内存/磁盘），不在远程装任何东西
npx winkops status -c ./wink.json --json

# logs：查看远程日志末 N 行，可选 grep 过滤
npx winkops logs /var/log/nginx/error.log -n 100 --grep timeout -c ./wink.json
# logs --follow：持续跟随新日志（tail -f），流式到 stdout，Ctrl-C 结束
npx winkops logs /var/log/nginx/error.log -f --grep timeout -c ./wink.json

# ps：远程进程快照（PID/属主/CPU/内存/命令），可选 --grep 按命令行过滤
npx winkops ps --grep node -c ./wink.json --json

# service：服务管理——status 只读放行；写动作（start/stop/restart/reload）须 --yes 确认
npx winkops service nginx status -c ./wink.json --json
npx winkops service nginx restart --yes -c ./wink.json --json
npx winkops service redis restart --manager docker --yes -c ./wink.json

# edit：守护式编辑远程配置——备份→替换→校验→reload→失败自动回滚
npx winkops edit /etc/nginx/nginx.conf --file ./nginx.conf \
  --validate 'nginx -t' --reload 'systemctl reload nginx' -c ./wink.json --json
```
...
```yaml
# wink.yaml — 与部署共用 connect，新增 stack 段
connect:
    host: 1.2.3.4
    port: 22
...
npx winkops provision -c ./wink.yaml --dry-run --json

# 确认执行（写操作，必须 --dry-run 预演或 --yes 确认其一，否则拒绝）
npx winkops provision -c ./wink.yaml --yes

# 只处理指定组件（位置参数限定子集）
npx winkops provision nodejs docker -c ./wink.yaml --yes
```
...
```bash
npx winkops -c ./wink.json --env prod   # 用 prod 的 host 与 remote，其余沿用基础配置
```
...
```bash
DEPLOY_PWD='***' npx winkops -c ./wink.json     # 或把 DEPLOY_PWD 写进 .env（已被 .gitignore）
```

> 这样配置文件可安全纳入版本管理，密码/密钥只存在于环境或未提交的 `.env` 中。

## 🙈 .winkignore（gitignore 风格忽略）

在本地根目录放一个 `.winkignore`，用 gitignore 语法（glob、目录、注释）排除不想上传的文件，比全字匹配的 `excludes` 更灵活；也可在配置 `sftpOptions.ignore` 里写内联规则，二者合并生效：
...
```bash
npx winkops -c ./wink.json --sftp-incremental
```
...
npx winkops -l dist -r /var/www/app -p 22 -u deploy --connect-private-key ~/.ssh/id_ed25519 \
...
```bash
npx winkops -c ./wink.json --sftp-backup --json
```

需要主动撤销上一次部署时，用 `--rollback` 恢复到最近一次快照：

```bash
npx winkops -r /var/www/app -h 1.2.3.4 -u deploy --connect-password '***' --rollback --json
```
...
```ts
import { SshSession, run, runMany, guard } from '@xwink/ops'
...
```bash
npx winkops -c ./wink.json --dry-run --json
```

`--json` 把结构化结果打到 **stdout**（人类日志一律走 stderr），因此可直接接 `jq`：

```bash
npx winkops -c ./wink.json --json | jq '.transferred'
```
...
- 实跑默认在 `~/.winkops/audit.log` 追加审计记录，便于事后排查；可用 `--audit-log` 改路径、`--no-audit` 关闭。
...
本包随附四个 Skill，让 Claude 直接按规约调用 `winkops`：

- **deploy** — 部署上传，封装「先预演 → 人工确认 → 真跑 → 按退出码/`ok`/`failed[]` 判定」的安全流程；覆盖增量、`.winkignore`/`ignore`、多环境 `--env`、YAML、`${ENV_VAR}` secrets。
...
- [ignore](https://github.com/kaelzhang/node-ignore) —— `.winkignore` 的 gitignore 风格匹配
...
欢迎 [提交 Issue](https://github.com/x-wink/winkops/issues) 反馈问题或提出建议。提交信息遵循 Conventional Commits。

## 📄 许可

[MIT](https://github.com/x-wink/winkops/blob/main/LICENSE) © XWINK
